...
#
# On Linux x86_64/aarch64 or Windows x86_64, LangSpark can install a native
# build instead (no Docker) from Preferences -> Study -> Language
# Installation, or via `langspark_core::install_voicevox_engine` — prefer
# that unless you're on macOS/another architecture, or specifically want the
# engine in Docker.
#
